...
省略 `model:=...` 时，launch 会读取 `rebotarm_hardware.yaml` 的 `default_model` 作为默认型号，控制节点（SDK 与硬件配置）和 RViz 的 URDF 加载都会跟随该值，保证两者一致。
...
`rebotarm_hardware.yaml` 顶部的 `default_model` 是整个工作空间的默认型号来源，默认值为 `dm`。如果希望 B601-RS 成为默认型号，可以改为：

```yaml
default_model: rs
```

修改后，未显式传入 `model:=...` 时，`bringup.launch.py`（控制节点与 RViz URDF）、`draw_square.launch.py`、`pick_place.launch.py` 以及 MoveIt 的 `demo.launch.py`、`hardware.launch.py` 都会自动读取该值并使用 B601-RS 配置，无需在每个命令中重复传入 `model:=rs`。显式传入 `model:=dm` 或 `model:=rs` 时仍会覆盖该默认值。
...
| `model` | `rebotarm_hardware.yaml` 的 `default_model` | 省略时回退到 `default_model`；本教程命令显式传入 `rs` 选择 B601-RS |
