...
`rebotarm_hardware.yaml` 顶部的 `default_model` 是整个工作空间的默认型号来源，默认值为 `dm`，因此本教程命令均省略 `model:=...`。如果需要切换为其他型号（如 `rs`），可以改为：

```yaml
default_model: rs
```

修改后，未显式传入 `model:=...` 时，`bringup.launch.py`（控制节点与 RViz URDF）、`draw_square.launch.py`、`pick_place.launch.py` 以及 MoveIt 的 `demo.launch.py`、`hardware.launch.py` 都会自动读取该值，无需在每个命令中重复传入 `model:=...`。显式传入 `model:=dm` 或 `model:=rs` 时仍会覆盖该默认值。
...
| `model` | `rebotarm_hardware.yaml` 的 `default_model` | 省略时回退到 `default_model`；本教程命令省略以使用 B601-DM |
